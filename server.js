var grezpay = require('./index.js');
const http = require('http');
const { parse } = require('querystring');

const hostname = '127.0.0.1';
const port = 3000;
const grezpay_payid = 'XXXXXXXXXXXXXXXX';
const grezpay_salt = 'XXXXXXXXXXXXXXXX';
const server = http.createServer((req, res) => {
    grezpay.isProdMode(false);
    grezpay.setCredentails(grezpay_payid, grezpay_salt);
    switch (req.url) {
        case "/":
        res.statusCode = 200;
        ...
        res.write('<title>Grezpay Demo Merchant Checkout Page</title>');
        ...
        res.write('<td class="form-control">');
        res.write('<input type="text" name="MERCHANTNAME" class="signuptextfield" value="Grezpay Demo" autocomplete="off"/>');
        res.write('</td>');
        res.write('</tr>');
        res.write('<tr>');
        res.write('<td class="labelfont">ORDER ID: </td>');
        res.write('<td class="form-control">');
        res.write('<input type="text" name="ORDER_ID" class="signuptextfield" value="GREZD1204191528" autocomplete="off" />');
        res.write('</td>');
        res.write('</tr>');
        res.write('<tr>');
        ...
        res.write('<td class="form-control">');
        res.write('<input type="text" name="CUST_NAME" class="signuptextfield" value="GREZPAY DEMO" autocomplete="off"/>');
        res.write('</td>');
        res.write('</tr>                   ');
        res.write('<tr>');
        ...
        res.write('<td class="form-control">');
        res.write('<input type="text" name="CUST_EMAIL" class="signuptextfield" value="test.user@grezpay.com" autocomplete="off"/>');
        res.write('</td>');
        res.write('</tr>');
        res.write('<tr>');
            ...
            collectRequestData(req, result => {
                if (result) {
                    var data = {
                        ...
                        PAY_ID: grezpay_payid,
                        PRODUCT_DESC: result.PRODUCT_DESC,
                        RETURN_URL: 'http://' + hostname + ':' + port + '/response',
                        TXNTYPE: 'SALE'
                    }
                    var transaction = grezpay.createTransaction(data);
                    var gateway_url = grezpay.getPaymentUrl();
